...
class StatusChecker:
    def __init__(self, cache_dir: Optional[str] = None):
        self.cache_dir = Path(cache_dir or os.environ.get('XDG_CACHE_HOME', '~/.cache')).expanduser() / 'dotfiles' / 'status'
    ...
    def get_packages_status(self, force_refresh: bool = False) -> Dict:
        ...
        return self._load_cache(self.packages_cache, {'packages': {}, 'total_updates': 0, 'last_check': 0, 'error': 'Cache unavailable'})
    ...
    def get_git_status(self, force_refresh: bool = False) -> Dict:
        ...
        return self._load_cache(self.git_cache, {'enabled': True, 'error': 'Git status unavailable'})
    ...
    def get_init_status(self, force_refresh: bool = False) -> Dict:
        ...
        return self._load_cache(self.init_cache, {'enabled': True, 'error': 'Init status unavailable'})
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                status_result = subprocess.run(['git', 'status', '--porcelain'],
                                             capture_output=True, text=True, timeout=5)
                data['uncommitted'] = len(status_result.stdout.strip().split('\n')) if status_result.stdout.strip() else 0
                ...
                if data['branch'] != 'detached':
                    try:
                        upstream_result = subprocess.run(['git', 'rev-parse', '--abbrev-ref', '@{upstream}'],
                                                       capture_output=True, text=True, timeout=5)
                        if upstream_result.returncode == 0:
                            upstream = upstream_result.stdout.strip()

                            counts_result = subprocess.run(['git', 'rev-list', '--left-right', '--count', f'{upstream}...HEAD'],
                                                         capture_output=True, text=True, timeout=5)
    ...
    def format_quiet_output(self, status: StatusResult) -> str:
        ...
        if total_updates > 0:
            if pkg_data.get('stale', False):
                messages.append(f"⚠️  {total_updates} package updates available (stale cache)")
            else:
    ...
    def format_interactive_output(self, status: StatusResult) -> str:
        ...
        if pkg_error:
            ...
            if total_updates > 0:
                if pkg_data.get('stale', False):
                    lines.append(f"   ⚠️  {total_updates} updates available (checking for new updates...)")
                else:
                    last_check = pkg_data.get('last_check', 0)
                    age = self._format_age(last_check)
        ...
        if init_data.get('enabled', False):
            ...
            if init_error:
                ...
                if init_data.get('needs_update', False):
                    age_hours = init_data.get('age_hours', 0)
                    lines.append(f"   ⚠️  Last run {int(age_hours / 24)}d ago - consider running")
                else:
