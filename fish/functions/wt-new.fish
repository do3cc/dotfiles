function wt-new --description "Create new worktree with proper organization"
    ...
    set branch $argv[3]  # source branch (default: main)

    if test (count $argv) -lt 2
        echo "Usage: wt-new <type> <name> [branch]"
        echo "Types: review, feature, bugfix, experimental"
        echo "Example: wt-new feature issue-25-logging branch-name"
        return 1
    end
    ...
    git worktree add "worktrees/$type/$name" $branch
    if test $status -eq 0
